...
function Recording:stopRecording()
    ...
    -- Check for final chunk generated by stopRecording
    local hasChunk = AudioRecorder.hasChunk()
    print("DEBUG: hasChunk=" .. tostring(hasChunk) .. " uploadEnabled=" .. tostring(uploadEnabled) .. " sessionId=" .. tostring(uploadSessionId))

    if hasChunk then
        local chunkData = AudioRecorder.getChunk()
        print("DEBUG: chunkData=" .. tostring(chunkData and #chunkData or "nil"))
        if chunkData and uploadEnabled and uploadSessionId then
            local chunkSeq = AudioRecorder.getChunkSequence()
            local queued = ChunkUploader.queueChunk(chunkData, chunkSeq)
            chunksQueued = chunksQueued + 1
            print("Final chunk " .. chunkSeq .. " queued (" .. #chunkData .. " bytes) result=" .. tostring(queued))
        else
            print("DEBUG: chunk not queued - chunkData=" .. tostring(chunkData ~= nil) .. " uploadEnabled=" .. tostring(uploadEnabled) .. " sessionId=" .. tostring(uploadSessionId ~= nil))
        end
    end
